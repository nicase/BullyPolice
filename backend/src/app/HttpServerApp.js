...
class HttpServerApp {
  constructor({
    config, logger, mongoose, express,
  }) {
    this.config = config;
    this.logger = logger;
  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      this.database
        .connect(this.config.database.url, { useNewUrlParser: true })
        .then(() => {
          this.logger.info(
          ...
          if (this.server === null) {
            ...
            const { hostname } = this.config.server;
            ...
            this.server.listen(port, hostname, () => {
              this.logger.info(
                `[p ${process.pid}] HTTP Server process created`,
              );
            });
            resolve();
          } else {
